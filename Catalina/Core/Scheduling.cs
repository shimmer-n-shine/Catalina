...
#pragma warning restore IDE0044 // Add readonly modifier
    ...
    public static void Setup(ServiceProvider services)
    {
        ...
        foreach (var @event in repeatingEvents)
        {
            ...
            {
                var nextExecution = DateTime.UtcNow.Round(TimeSpan.FromSeconds((ulong)attribute.AlignTo));
            ...
            if (utcNow >= @event.NextExecution)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    @event.NextExecution =
                        ...
                        ? utcNow.Round(re.Interval)
                        : utcNow.Round(TimeSpan.FromHours(1));
                }
                if (!success) continue;
                ...
                {
                    @event.NextExecution = (utcNow.Round(repeatingEvent.Interval));
                    services.GetRequiredService<Logger>()
